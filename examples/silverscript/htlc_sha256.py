"""TN10 SilverScript SHA256 HTLC: genesis + hashlock claim (refund after refund_daa).

SilverScript cannot pass `bytes` into build_sig_script_for_covenant_decl yet; the
demo reveals preimage bytes as int octets (p0..p3) and checks sha256 at compile time.
Pairs with examples/galleon HtlcBridgeReleaseSha256 (bytes32 hashlock).

  python examples/silverscript/htlc_sha256.py --print-address
  python examples/silverscript/htlc_sha256.py --wallet grace
"""
...
# Demo preimage b"HTLC" and its SHA256 digest (matches L2 HtlcBridgeReleaseSha256).
PAYMENT_PREIMAGE = b"HTLC"
PAYMENT_HASH_SHA256 = hashlib.sha256(PAYMENT_PREIMAGE).digest()
PAYMENT_HASH_HEX = "0x" + PAYMENT_HASH_SHA256.hex()
PREIMAGE_HEX = "0x" + PAYMENT_PREIMAGE.hex()
PREIMAGE_BYTES = list(PAYMENT_PREIMAGE)
FLOW_CLAIM = ("genesis", "claim")
FLOW_REFUND = ("genesis", "refund")
PROOF_PATH = LOCAL / "tn10-htlc-sha256-proof.json"
FIXTURE_PROOF = ROOT / "fixtures" / "tn10-htlc-sha256-proof.json"
REFUND_PROOF_PATH = LOCAL / "tn10-htlc-sha256-refund-proof.json"
FIXTURE_REFUND_PROOF = ROOT / "fixtures" / "tn10-htlc-sha256-refund-proof.json"
...
contract HtlcSha256(int refund_daa) {{
 int status = 0;

 #[covenant(binding = auth, from = 1, to = 1, mode = transition)]
 function claim(State prev_state, int p0, int p1, int p2, int p3, int current_daa) : (State) {{
 require(prev_state.status == 0);
{checks}
 require(sha256({PREIMAGE_HEX}) == {PAYMENT_HASH_HEX});
...
@lru_cache(maxsize=8)
def compiled_htlc(refund_daa: int) -> silverscript.CompiledContract:
    return silverscript.compile(_source(), [refund_daa])


def lock_script(refund_daa: int, status: int) -> ScriptPublicKey:
    contract = compiled_htlc(refund_daa)
...
def htlc_address(refund_daa: int, status: int = 0) -> Address:
    return address_from_script_public_key(lock_script(refund_daa, status), NETWORK_TYPE)
...
def unlock_script(
    ...
    if function == "claim":
        call = contract.build_sig_script_for_covenant_decl(
            "claim", PREIMAGE_BYTES + [current_daa]
        )
    ...
    state_bytes = script_with_int_state(contract, status)
...
class HtlcState:
    ...
    def utxo(self) -> UtxoEntryReference:
        spk = lock_script(self.refund_daa, self.status)
        addr = htlc_address(self.refund_daa, self.status)
...
async def build_htlc_tx(
    ...
    *,
    ...
    spk = lock_script(refund_daa, status)
...
async def claim(client: RpcClient, state: HtlcState) -> HtlcState:
    ...
    spend = TransactionInput(
        state.outpoint,
        unlock_script(state.refund_daa, state.status, "claim", current_daa),
        sequence=0,
        sig_op_count=0,
    ...
    tx, value = await build_htlc_tx(
        client, [spend], state.value, state.refund_daa, 1, binding
    )
...
async def refund(client: RpcClient, state: HtlcState) -> HtlcState:
    ...
    spend = TransactionInput(
        state.outpoint,
        unlock_script(state.refund_daa, state.status, "refund", current_daa),
        sequence=0,
        sig_op_count=0,
        compute_budget=COMPUTE_BUDGET,
        utxo=(
    ...
    binding = CovenantBinding(authorizing_input=0, covenant_id=Hash(state.covenant_id))
    tx, value = await build_htlc_tx(
        client, [spend], state.value, state.refund_daa, 2, binding
    )
    result = await submit_transaction(client, {"transaction": tx, "allowOrphan": False})
    return HtlcState(
        result["transactionId"],
...
def show_step(label: str, state: HtlcState) -> None:
    ...
    print(f"  payment_hash {PAYMENT_HASH_HEX}")
    print(f"  preimage    {PAYMENT_PREIMAGE!r}")
    print(f"  refund_daa  {state.refund_daa}")
    print(f"  address     {htlc_address(state.refund_daa, state.status)}")
...
async def recover_last(client: RpcClient, last: dict, refund_daa: int) -> HtlcState:
    state = HtlcState(
        str(last["txid"]),
    ...
    addresses = [htlc_address(refund_daa, state.status)]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TN10 SilverScript SHA256 HTLC reference")
...
async def main() -> None:
    ...
    print(f"HTLC open address (status=0): {htlc_address(refund_daa, 0)}\n")
    ...
    if not remaining:
        ...
        if changed:
            write_htlc_proof(proof_path, steps, funding_text, refund_daa)
    ...
    print(
        f"HtlcSha256 on {NETWORK_ID}  branch={flow[-1]}  payment_hash={PAYMENT_HASH_HEX}  "
    ...
    try:
        ...
        if steps:
            htlc = await recover_last(client, steps[-1], refund_daa)
        ...
        if remaining[0] == "genesis":
            ...
            attach_live_utxo(
                ...
                await wait_until_accepted(
                    client, htlc.txid, htlc_address(refund_daa, 0)
                ),
            )
        ...
        if "claim" in remaining:
            ...
            attach_live_utxo(
                htlc,
                await wait_until_accepted(
                    client, htlc.txid, htlc_address(refund_daa, htlc.status)
                ),
            )
