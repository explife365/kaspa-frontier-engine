...
Copy-paste intro for Discord or GitHub discussions: [`scripts/kaspa_dev_share_post.txt`](scripts/kaspa_dev_share_post.txt). Toccata integrator post: [`scripts/kaspa_community_toccata_post.txt`](scripts/kaspa_community_toccata_post.txt). Skynet vs community momentum: [`scripts/kaspa_community_social_gap_post.txt`](scripts/kaspa_community_social_gap_post.txt). SDK `computeBudget` gate: `python scripts/tn10_sdk_gate.py --json`. Nudge text for [kaspa-python-sdk PR #78](https://github.com/kaspanet/kaspa-python-sdk/pull/78): [`scripts/sdk_pr78_pr_comment.txt`](scripts/sdk_pr78_pr_comment.txt). CEX production checklist: [`scripts/cex_production_runbook.md`](scripts/cex_production_runbook.md). `getUtxosByCovenantId` KIP outline: [`scripts/getUtxosByCovenantId_kip_outline.md`](scripts/getUtxosByCovenantId_kip_outline.md). Foundation Skynet draft (ops only, not a dev wake-up): [`scripts/certik_foundation_submission.md`](scripts/certik_foundation_submission.md).
...
| Silverscript apps with public TN10 txids | Toccata docs | `counter.py` + `timelock_vault.py` + `restricted_swap.py` with checked-in `fixtures/tn10-{counter,vault,swap}-proof.json` (+ REST/kascov snapshots). Native broadcast stays fail-closed until kaspa-python-sdk#78 publishes; TN10 dev rehearsal: `TN10_SDK_DEV_PATCH=1` + `scripts/tn10_dev_broadcast.ps1`. `tn10-proof` + `scripts/tn10_covenant_rehearsal.ps1` |
