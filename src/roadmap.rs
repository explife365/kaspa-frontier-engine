...
pub fn print_integrator_next() {
    ...
    println!("  CEX rehearsal: bounded multi-address ingestion + durable exact withdrawals + leased idempotency-key webhook outbox + delta-driven durable wRPC + owned-node resnapshots + supervisor health gate. Production still needs redundant node ops and a deduplicating receiver");
}
